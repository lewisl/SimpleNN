...
function (layer::ConvLayer)(x::AbstractArray{ELT,4})
    @views layer.a_below[:,:,:,:] .= x   # as an alias, this might not work for backprop though it seems to
    pad = layer.pad
    # mb_rng = layer.mb_rng[]
    ...
    @turbo for b in axes(layer.z, 4)  #  mb_rng  # batch
...
function (layer::ConvLayer)(layer_above)
    ...
    @views layer_above.eps_l[:,:,:, :] .*= layer.grad_a[:,:,:, :]
    ...
    elseif layer.padrule == :same
        @views layer.pad_above_eps[:,:,:, :] .= layer_above.eps_l[:,:,:, :]
    end
    ...
    @turbo for b in axes(layer.eps_l, 4)     #   axes(layer.eps_l, 4)  mb_rng
...
function (layer::MaxPoolLayer)(layer_above)
    ...
    (pool_h, pool_w) = layer.pool_size
    # mb_rng = layer.mb_rng[]
    @inbounds for bn in axes(layer_above.eps_l, 4)    #   mb_rng   # @inbounds
...
function (layer::FlattenLayer)(x::AbstractArray{ELT,4})
    h, w, ch, _ = size(x)
    # mb_rng = layer.mb_rng[]
    for b in axes(x, 4)    #  mb_rng  # iterate over batch dimension
...
function (layer::FlattenLayer)(layer_above::LinearLayer)
    ...
    @views mul!(layer.dl_dflat[:, :], layer_above.weight', layer_above.eps_l[:, :])  # in-place matrix multiplication

    # Reshape in-place using explicit indexing
    h, w, ch, batch_size = size(layer.eps_l)
    for b in 1:batch_size   #   mb_rng
        idx = 1
        @turbo for c in 1:ch
            for j in 1:w
                for i in 1:h
...
function (layer::LinearLayer)(x::Matrix{ELT})
    ...
    @views mul!(layer.z[:, :], layer.weight, x)  # in-place matrix multiplication

    # bias: explicit loop faster than broadcasting
    if layer.dobias
        @turbo for j in axes(layer.z, 2)   #   mb_rng # For each column (sample)
    ...
    @views layer.a_below[:, :] .= x   # assign alias for using in backprop
...
function (layer::LinearLayer)(layer_above::LinearLayer)
    ...
    if layer.isoutput
        ...
        @views mul!(layer.grad_weight, layer.eps_l[:, :], layer.a_below[:,:]')  # in-place matrix multiplication
        ...
        @views mul!(layer.eps_l[:, :], layer_above.weight', layer_above.eps_l[:, :])  # in-place matrix multiplication
        @views layer.eps_l[:, :] .*= layer.grad_a[:, :]  # in-place element-wise multiplication
    ...
    if layer.dobias
        ...
        @turbo for j in axes(layer.eps_l, 2)  # axes(layer.eps_l, 2)  mb_rng
