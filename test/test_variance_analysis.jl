...
@testset "Variance Analysis Tests" begin
    ...
    @testset "analyze_regression_variance function" begin
        ...
        # Create mock layer with the true weights (1, n_features) format
        ...
        @test haskey(results, "partial_r2")
        @test haskey(results, "semi_partial_r2")
        ...
        @test length(results["partial_r2"]) == 3
        @test length(results["semi_partial_r2"]) == 3
        ...
        @test all(results["partial_r2"] .>= 0)
    @testset "Classic R² equals sum of semi-partial R² (independent features)" begin
        n_samples = 10000
        Random.seed!(42)
        # Independent features with differing variances
        X = [randn(n_samples)'; 0.5 .* randn(n_samples)'; 2.0 .* randn(n_samples)']
        true_weights = [1.2, -0.7, 0.5]
        true_bias = 0.3
        ...
        # Compute R² using the same metric function
        y_pred = mock_layer.weight * X .+ mock_layer.bias
        classic_r2 = calculate_r2(y_pred, y)
        sum_semi = sum(results["semi_partial_r2"])

        # For independent predictors, sum of semi-partial R² approximates overall R²
        @test isapprox(classic_r2, sum_semi; atol=0.03)
        # Classic R² should match the overall R² we compute from predictions
        @test isapprox(classic_r2, results["overall_r2"]; atol=1e-10)
    end

        @test all(results["semi_partial_r2"] .>= 0)
    end
    ...
    @testset "print_variance_analysis function" begin
        ...
        test_results = Dict(
            ...
            "partial_r2" => [0.2, 0.3, 0.2],
            "semi_partial_r2" => [0.25, 0.35, 0.25],
